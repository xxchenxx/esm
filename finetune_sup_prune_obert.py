...
def main(args):
    ...
    def loss(model_outputs, loss_target):
        hidden = model_outputs['hidden']
        logits = linear(hidden)
        labels = torch.tensor(loss_target).cuda().long()
        loss = (torch.nn.functional.cross_entropy(logits.reshape(-1, args.num_classes), labels.reshape(-1)))
        return loss  
    def _data_loader_builder(kwargs):
        while True:  # infinite dataloading
            for batch_idx, (labels, strs, toks) in enumerate(train_data_loader):
                ...
                yield [], toks, labels    
    optimizer = manager.modify(model, optimizer, steps_per_epoch, grad_sampler={
        "data_loader_builder": _data_loader_builder,
        "loss_function": loss ,
    },)
