...
def run(args):
    ...
    # )
    data_loader = torch.utils.data.DataLoader(
        dataset, collate_fn=alphabet.get_batch_converter(args.truncation_seq_length, with_attention=True), shuffle=True, batch_size=1
    )
    ...
    # config.cross_attention_hidden_size = 1280
    ...
    no_decay = ["bias", "LayerNorm.weight"]
    esm_optimizer_grouped_parameters = [
        {
    ...
    optimizer_esm = AdamW(esm_optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon, betas=(args.beta1,args.beta2))
    scheduler = get_linear_schedule_with_warmup(
        optimizer, num_warmup_steps=warmup_steps, num_training_steps=t_total
    )

    scheduler_esm = get_linear_schedule_with_warmup(
        optimizer, num_warmup_steps=warmup_steps, num_training_steps=t_total
    )
