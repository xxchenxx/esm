...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def prepare_inputs_for_generation(self, input_ids, **kwargs):
        ...
        print({"input_ids": input_ids, "encoder_hidden_states": encoder_hidden_states})
        assert False
    ...
    def generate(
        ...
        if num_return_sequences != 1:
            ...
            if encoder_hidden_states is not None:
                encoder_hidden_states = encoder_hidden_states.unsqueeze(1).expand(batch_size, num_return_sequences, *encoder_hidden_states.shape[1:])
                encoder_hidden_states = encoder_hidden_states.contiguous().view(
                    batch_size * num_return_sequences, *encoder_hidden_states.shape[2:]
                )
    ...
    def _generate_beam_search(
        ...
        if encoder_hidden_states is not None:
            encoder_hidden_states = encoder_hidden_states.unsqueeze(1).expand(batch_size, num_beams, *encoder_hidden_states.shape[1:])
            encoder_hidden_states = encoder_hidden_states.contiguous().view(
                batch_size * num_beams, *encoder_hidden_states.shape[2:]
            )
        ...
        while cur_len < max_length:
            ...
            if past:
                ...
                for layer_past in past:
                    ...
                    for i in beam_idx:
                        reordered_layer_past.append(layer_past[:, i].unsqueeze(1).clone().detach())
