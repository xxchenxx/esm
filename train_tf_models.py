...
def run(args):
    ...
    for i in range(args.steps):
        for batch_idx, (labels, _, toks, mask, type_id) in enumerate(data_loader):
            ...
            encoder_hidden_states = out["representations"][repr_layers[-1]]
            # encoder_hidden_states: N x hidden_dim
            ...
            if global_step > args.steps:
                break
        torch.save({'state_dict': decoder.state_dict(), 'steps': global_step}, args.output_name + ".pt") 
        if global_step > args.steps:
            break
    torch.save({'state_dict': decoder.state_dict(), 'steps': global_step}, args.output_name + ".pt") 
